...
- Verify that `tree_head.size` is greater than the size of the tree head
...
- Verify `combined` as executing the following proofs in this order:
  ...
  - If a provisional tree head was advertised by the client and the subsequent
  ...
  - Finally, a `PrefixProof` from the rightmost log entry is added to
    `combined`.
- The `sequencing` field must be present when a provisional tree head was
  ...
  - Verify that `sequencing.size` is greater than or equal to the retained size
    of the Certificate Subtree.
  - Verify that `sequencing.{first_valid, invalid_entries}` would not unrevoke
    any previously-revoked certificates.
- If the `sequencing` field is present, compare the following fields in
  ...
  - Verify that `subtree.{first_valid, invalid_entries}` would not unrevoke any
...
- Compute the root hash of the Log Tree and verify the signature in
  `tree_head.signature`:
  - Compute the root hash of the Certificate Subtree as it exists in the
  ...
  - With the root hash of the Certificate Subtree and the other fields in
  ...
  - With the Prefix Tree leaf hash(es), compute the root hash of the Log Tree
...
- Verify that `subtree.{first_valid, invalid_entries}` do not exclude
  `subtree.position`.
- Compute the root hash of the Log Tree and the root hash of the provisional
  Prefix Tree:
  - Compute the root hash of the Certificate Subtree as it exists in the
  ...
  - Compute the root hash of the Log Tree with `combined` (which may require the
  ...
  - With the provisional Prefix Tree leaf hash and the proof in `prefix`,
...
- Verify that the client advertised a standard tree head type for the chosen
  Transparency Log.
...
- Verify that `subtree.{first_valid, invalid_entries}` do not revoke
  `subtree.position`.
- Compute the root hash of the Prefix Tree and verify that it matches the
  retained value:
  ...
  - With the Prefix Tree leaf hash and the proof in `prefix`, compute the Prefix
    Tree root hash.
...
- Verify that the client advertised a provisional tree head type for the chosen
  Transparency Log.
- For the advertised tree head, verify that the rightmost log entry's timestamp
  is within the bounds set by `max_ahead` and `max_behind`.
- Verify that the previously-retained `first_valid` and `invalid_entries` fields
  do not revoke `position`.
- Compute the root hash of the Certificate Subtree by interpreting `inclusion`
